...
[m1, t1, a1, s1] = loadFile(strcat(rootDirName, dirName, '\posture_contact\data.scs2.mat'), true);
...
% deltaMPercentage = deltaM ./ m0Sample;
% plot(tSample, deltaM);
% 
% disp(max(deltaMPercentage));
...
m_h = 0.15;
p3 = line([6 21], [m_h m_h], 'LineWidth', lineWidth * 0.3, 'Color', [0 0 0], 'LineStyle', '--');
...
lgd = legend([p0 p1 p2 p3], labels, 'Interpreter', 'latex', 'FontSize', 16);
...
% minIndexAdditional = getMinIndex(t, 4.0, Inf);
% maxIndexAdditional = getMaxIndex(t, 0.0, 24.0);
% 
